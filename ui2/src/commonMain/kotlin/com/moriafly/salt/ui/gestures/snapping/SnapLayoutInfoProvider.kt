...
/**
 * Provides information about the layout that is using a [androidx.compose.foundation.gestures.snapping.snapFlingBehavior]. The provider should
...
interface SnapLayoutInfoProvider {
     ...
     * this is [decayOffset] a suggested offset given by [androidx.compose.foundation.gestures.snapping.snapFlingBehavior] to indicate where the
     * animation would naturally decay if using [velocity]. Returning a value higher than
     * [decayOffset] is valid and will force [androidx.compose.foundation.gestures.snapping.snapFlingBehavior] to use a target based animation
     * spec to run the approach phase since we won't be able to naturally decay to the proposed
     * offset. If a value smaller than or equal to [decayOffset] is returned [androidx.compose.foundation.gestures.snapping.snapFlingBehavior]
